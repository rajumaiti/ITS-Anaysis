##### DW plots for Maharashtra data-set #####
...
data=read_excel("the path for the required file", sheet = "MAHARASHTRA")
View(data)
str(data)

data=data %>% select(DATE,CONFIRMED, DECEASED) %>% filter(DATE >= "2021-03-15" & DATE < "2021-06-19")
...
data=add_col_data(data, "2021-04-14")
...
date.list=seq(as.Date("2021-06-19"),  as.Date("2021-06-25"), by= 1)
reg1=list()
reg2=list()

for (i in seq_along(date.list)){
  dt=read_excel("# the path for the required file", sheet = "MAHARASHTRA")
  dt$DATE=as.Date(dt$DATE)
  dt=dt %>% select(DATE,CONFIRMED, DECEASED) %>% filter(DATE >= "2021-03-15" & DATE <= date.list[i])
  dt=add_col_data(dt, "2021-04-14")
...
plt1=plt1+labs(title = "Daily cases (log) in Maharashtra")+theme(plot.title = element_text(face = "bold",size = 10,hjust = 0.5),axis.title = element_text(size = 10),
...
plt2=plt2+labs(title = "Daily deaths (log) in Maharashtra")+theme(plot.title = element_text(face = "bold",size = 10,hjust = 0.5),axis.title = element_text(size = 10),
                                                               axis.text = element_text(size = 10),
                                                               legend.text=element_text(size=10),
